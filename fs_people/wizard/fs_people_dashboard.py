...
class FsPeopleDashboard(models.TransientModel):
    ...
    def _compute_summary_kpis(self):
        ...
        for record in self:
            ...
            i_expired = Instructor.search_count([
                '|', ('medical_status', '=',
                      'expired'), ('has_expired_qualification', '=', True)
            ])
            s_expired = Student.search_count(
                [('has_expired_status', '=', True)])
            p_expired = Pilot.search_count(
                [('has_expired_qualification', '=', True)])
            total = i_total + s_total + p_total
            expired = i_expired + s_expired + p_expired
            record.personnel_total = total
            record.compliance_health = (
    ...
    def _compute_instructor_graph_data(self):
        ...
        data = [
            {'label': 'Valid', 'value': Instructor.search_count(
                [('medical_status', '=', 'valid')]), 'type': 'future'},
            {'label': 'Expiring', 'value': Instructor.search_count(
                [('medical_status', '=', 'expiring')]), 'type': 'past'},
            {'label': 'Expired', 'value': Instructor.search_count(
                [('medical_status', '=', 'expired')]), 'type': 'past'},
        ]
        for record in self:
